...
const ROAD={
 corp:c=>({label:'기술창업 로드맵', cur:1, stages:[
   ...
   {n:'성장', d:'매출과 고용', items:[['창업도약패키지','cond','업력 3년 이후'],
     ['TIPS','cond','운영사 추천 필요'],['고용창출장려금','free'],['이노비즈 인증','cond','업력 3년 이후']]},
 ...
 worker:c=>({label:'직장인 여정', cur:c.work.insured<365?0:1, stages:[
   {n:'입사', d:'취업 직후', items:[['고용보험 가입','done'],
     ['소득세 감면 신청', c.work.taxRelief?'done':'now'],
     ['청년내일채움공제','lost']]},
   {n:'재직', d:'상시 가능', items:[
...
const EVENTS={
 heir:c=>({label:'상속 절차', urgent:true, sub:`사망일로부터 ${c.event.deathDays}일 경과`,
   items:[
     ...
     ['안심상속 원스톱 서비스', c.event.deathDays>30?'lost':'now','사망 후 1개월 이내',
      c.event.deathDays>30?'기간 경과':`${Math.max(0,30-c.event.deathDays)}일 남음`],
...
const CROSS={
 d:[['실업급여 신청이 막힐 수 있습니다',
    ...
    ['폐업 순서를 잘못 잡으면 지원을 놓칩니다',
     '희망리턴패키지는 폐업 신고 전에 상담을 받아야 합니다. 실업급여를 위해 먼저 폐업하면 이 지원을 받을 수 없습니다']],
 ...
 h:[['사업자등록 시점이 지원 규모를 가릅니다',
     '지금은 예비창업패키지 최대 1억 원 대상입니다. 사업자등록을 먼저 하면 이 자격이 영구히 사라지고 초기창업패키지로만 갈 수 있습니다'],
 ...
 i:[['상속 기한이 두 개 동시에 흐릅니다',
    ...
    ['안심상속을 먼저 해야 판단할 수 있습니다',
     '고인의 재산과 채무를 모르면 상속포기 여부를 결정할 수 없습니다. 안심상속 원스톱 서비스는 사망 후 1개월 이내에만 신청할 수 있습니다']],
...
/* ── 알림 ── */
const NOTI={
 a:[['초기창업패키지 마감','12일 남았습니다 · 선정 가능성 52%','오늘','warn'],
 ...
 d:[['실업급여 신청 전 확인','사업자 휴업 또는 폐업 처리가 선행돼야 합니다','오늘','stop'],
    ...
    ['희망리턴패키지','폐업 신고 전에 상담을 받아야 합니다','2일 전','warn']],
 e:[['한부모 아동양육비 월 42만','자녀 2명 · 소득 요건 충족 · 미신청','오늘','stop'],
 ...
 i:[['상속포기 · 한정승인','52일 남았습니다 · 채무 확인 후 결정','오늘','stop'],
    ['안심상속 원스톱','신청 기간 8일이 지났습니다','기록','stop'],
...
const ASK={
 corp:[{q:'지금 내가 받을 수 있는 게 뭐야?',mode:'logic',
   a:'선정 가능성이 40% 이상인 것은 초기창업패키지와 청년창업사관학교입니다. 창업성장 R&D는 자격은 되지만 기술성 증빙이 약해 23%로 낮습니다. 국세 미환급금 37만 원도 조회됐습니다.',
   cite:'저장된 개업일 · 업종 · 상시근로자 · 특허 상태로 계산'},
  {q:'직원 한 명 더 뽑으려는데 괜찮아?',mode:'logic',
   a:'채용 전에 워크넷 구인등록을 먼저 하셔야 합니다. 등록 없이 채용하면 고용창출장려금 대상에서 제외되고, 두루누리는 채용월을 포함해 신청해야 소급됩니다.',
 ...
 solo:[{q:'장사가 안 되는데 접어야 할까?',mode:'logic',
   a:'폐업을 결정하시기 전에 희망리턴패키지 상담을 먼저 받으셔야 합니다. 폐업 신고를 하고 나면 일부 지원을 신청할 수 없습니다.',
   cite:'저장된 매출 추이 · 폐업 관련 제도 요건으로 계산'},
 ...
 free:[{q:'사업자등록 하려는데 괜찮아?',mode:'logic',
   a:'등록하시기 전에 예비창업패키지를 먼저 확인하셔야 합니다. 최대 1억 원 규모인데 사업자등록 전에만 신청할 수 있고, 등록하는 순간 자격이 영구히 사라집니다.',
...
/* ── 진행 ── */
const RUN={
 corp:{title:'창업성장 R&D', amt:'최대 2억', dday:'마감 12일 남음',
   lead:'감점 항목을 먼저 해결하도록 순서를 잡았습니다',
   steps:[['창업교육 이수','온라인 4시간 · 서류평가 가점','now'],
          ['특허 등록 전환 검토','R&D 선정 가능성 23% → 47%','now'],
 ...
 solo:{title:'경영안정자금', amt:'최대 7,000만', dday:'예산 62% 소진',
 ...
 online:{title:'희망리턴패키지', amt:'최대 2,000만', dday:'폐업 신고 전에만',
   lead:'폐업을 신고하고 나면 신청할 수 없습니다',
   steps:[['사전 상담 예약','소진공 지역센터 · 폐업 신고 전 필수','now'],
          ['사업 정리 컨설팅','재고 · 임차 · 세무 정리','wait'],
          ['폐업 신고','상담 이후에 진행','wait'],
          ['재취업 지원 연계','실업급여 자격 확인 포함','wait']], paid:0},
 free:{title:'예비창업패키지', amt:'최대 1억', dday:'사업자등록 전에만',
 ...
 worker:{title:'중소기업 취업자 소득세 감면', amt:'연 200만 한도 · 90%', dday:'제도 일몰 3개월 전',
 ...
 seeker:{title:'실업급여', amt:'150일 지급', dday:'이직일로부터 12개월 이내',
 ...
 family:{title:'주거 지원 확인', amt:'유형별 상이', dday:'공고별',
